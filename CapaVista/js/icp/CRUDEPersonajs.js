...
function AddrowPersona(data) {            
    ...
    for (var contPersona = 0; contPersona < data.length; contPersona++) {                         
        tablaPersona.row.add([
            ...
            '<button value="add" href="#"  title="add" class="btn btn-success btn-addEmpleado"><i class="fa fa-plus" ></i> </button>',
...
function VerificarExistePersona() {               
    if ($('#txtNuevoNombre1').val().length >= 3 &&  $('#txtNuevoApellido1').val().length >= 3  &&$('#ddlCPersonaGenero').val() > 0) {                            
        return true;
    }
    else {
        return false;
    ...
    $.ajax({
        ...
        success: function (data) {
            if (VAlDDLPersonaGenero == "null") {
                $('#ddlCPersonaGenero').append($("<option> </option>").val("0").html("Seleccionar Género"));                                                  
            }
            else {
                $.each(data.d, function (data, value) {
                    if (VAlDDLPersonaGenero == value.Genero) {
                        $('#ddlCPersonaGenero').append($("<option> </option>").val(value.IdGenero).html(value.Genero));           
...
function FnAlertaPersona() {            
    ...
    $('#alertaEmpleados .modal-content').addClass(VarJsColorAlertPersona);            
    $('#alertaEmpleados h5').text(VarJsTextoAlertPersona);            
    $('#alertaEmpleados').modal('show');
    setTimeout(function () {
        $('#alertaEmpleados').modal('hide');
        $('#alertaEmpleados .modal-content').removeClass(VarJsColorAlertPersona);            
    }, 1500);                           
